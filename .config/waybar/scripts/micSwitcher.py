...
# function to parse output of command "wpctl status" and return a dictionary of sources with their id and name.
...
wofi_command = f"echo '{output.strip()}' | rofi -dmenu -i -theme ~/.config/rofi/launchers/type-1/style-8-corner.rasi -window-title \"Audio Input Switcher\" -markup-rows -selected-row {position} -a {position} -p \"Choose audio output\" -l 4"
